...
function renderDashboard(){
  ...
  const stats = [
    { n: captains.length, l: 'إجمالي الكباتن' },
    { n: captains.filter(c => c.status === 'new').length, l: 'كباتن جدد' },
    { n: captains.filter(c => c.status === 'active').length, l: 'كباتن نشطون', cls:'ok' },
    { n: captains.filter(c => c.status === 'needs_followup').length, l: 'يحتاجون متابعة', cls:'warn' },
    { n: todayTasks.length, l: 'مهام اليوم' },
    { n: overdueTasks.length, l: 'مهام متأخرة', cls:'warn' },
    { n: doneThisWeek.length, l: 'مهام مكتملة (٧ أيام)', cls:'ok' },
    { n: notesToday.length, l: 'ملاحظات اليوم' },
  ];
...
profileStatusEl.innerHTML = STATUS_OPTIONS.map(s => `<option value="${s.id}">${s.label}</option>`).join('');
